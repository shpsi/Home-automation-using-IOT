...
void loop() {  
  ...
  Serial.println("isOrder is " + String(result));
  if(result==0){
    ...
    if(luxInput>0&&luxInput<=lxM){     
      digitalWrite(LED,1);  
      String url61 = "http://172.20.43.132/smartLight/smartLightLocal/setonOff.php?&nId=" + String(node_id) + "&onOff=1";
      WiFiClient client;
      const int httpPort = 80;
      if (!client.connect(host, httpPort)) {
       Serial.println("connection failed 2");
      ...
      client.print(String("GET ") + url61 + " HTTP/1.1\r\n" +"Host: " + host + "\r\n" +"Connection: close\r\n\r\n");
      delay(1000);   
    }
    else{
      digitalWrite(LED,0);
